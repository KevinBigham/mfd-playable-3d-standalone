...
interface PartOpts { color: THREE.Color; bone: number; surf?: Surface; bevel?: number }
...
export function buildAthleteRig(
  ...
  // hips / pants
  rb.add(chunk(0.62 * bulk, 0.34, 0.42 * bulk, seg), place(0, 0, 0), { color: pants, bone: B.hips, surf: SURF.PANTS });
  // torso + jersey
  rb.add(chunk(0.66 * bulk, 0.62, 0.44 * bulk, seg), place(0, 0.18, 0), { color: jersey, bone: B.chest, surf: SURF.JERSEY });
  // shoulder pads — the silhouette maker
  rb.add(chunk(shoulderW * 2.32, 0.30, 0.52 * bulk, seg), place(0, 0.46, 0), { color: jersey, bone: B.chest, surf: SURF.JERSEY });
  rb.add(chunk(shoulderW * 2.32 * 0.92, 0.10, 0.54 * bulk, seg), place(0, 0.60, 0), { color: accent, bone: B.chest, surf: SURF.TRIM });
  // chest stripe
  rb.add(chunk(0.30, 0.30, 0.10, 1), place(0, 0.24, 0.22 * bulk), { color: ink, bone: B.chest, surf: SURF.TRIM });

  // neck + helmet
  rb.add(chunk(0.22, 0.14, 0.22, seg), place(0, 0.02, 0), { color: skin, bone: B.neck, surf: SURF.SKIN });
  const headGeo = new THREE.SphereGeometry(0.30, seg * 5 + 5, seg * 4 + 4);
  headGeo.scale(1.06, 1.0, 1.12);
  rb.add(headGeo, place(0, 0.06, 0), { color: helmet, bone: B.head, surf: SURF.HELMET });
  // helmet stripe
  const stripe = new THREE.BoxGeometry(0.085, 0.30, 0.62);
  rb.add(stripe, place(0, 0.22, 0.0), { color: accent, bone: B.head, surf: SURF.TRIM });
  // facemask
  const mask = new THREE.TorusGeometry(0.19, 0.032, 4, 10, Math.PI);
  mask.rotateY(Math.PI / 2); mask.rotateZ(Math.PI / 2);
  rb.add(mask, place(0, -0.02, 0.24), { color: dark, bone: B.head, surf: SURF.METAL });
  const bar = new THREE.CylinderGeometry(0.028, 0.028, 0.34, 5);
  bar.rotateZ(Math.PI / 2);
  rb.add(bar, place(0, 0.02, 0.30), { color: dark, bone: B.head, surf: SURF.METAL });
  if (def.flair === 3) {
    const visor = new THREE.SphereGeometry(0.28, 10, 6, 0, Math.PI, 0.6, 0.7);
    rb.add(visor, place(0, 0.08, 0.02), { color: dark, bone: B.head, surf: SURF.VISOR });
  }
  ...
  for (const side of [-1, 1]) {
    ...
    rb.add(chunk(0.24 * bulk, 0.44, 0.24 * bulk, seg), place(0, -0.20, 0), { color: sleeve, bone: bS, surf: def.flair === 4 ? SURF.JERSEY : SURF.SKIN });
    rb.add(chunk(0.21 * bulk, 0.40, 0.21 * bulk, seg), place(0, -0.19, 0), { color: skin, bone: bE, surf: SURF.SKIN });
    // oversized hands
    rb.add(chunk(0.24, 0.24, 0.16, seg), place(0, -0.10, 0.02), { color: def.flair === 2 ? accent : skin, bone: bH, surf: def.flair === 2 ? SURF.TRIM : SURF.SKIN });
    if (def.flair === 2) rb.add(chunk(0.24 * bulk, 0.09, 0.24 * bulk, 1), place(0, -0.40, 0), { color: accent, bone: bE, surf: SURF.TRIM });
  }
  ...
  for (const side of [-1, 1]) {
    ...
    rb.add(chunk(0.30 * bulk, 0.48, 0.30 * bulk, seg), place(0, -0.22, 0), { color: pants, bone: bT, surf: SURF.PANTS });
    rb.add(chunk(0.25 * bulk, 0.46, 0.25 * bulk, seg), place(0, -0.21, 0), { color: def.flair === 5 ? ink : skin, bone: bK, surf: def.flair === 5 ? SURF.PANTS : SURF.SKIN });
    rb.add(chunk(0.26, 0.13, 0.42, seg), place(0, -0.06, 0.06), { color: dark, bone: bF, surf: SURF.CLEAT });
  }
